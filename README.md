...
3. В разделе «Доступы» найдите **Яндекс.Вебмастер** и отметьте
   **`webmaster:hostinfo`** (просмотр информации о сайтах и статистики).
   ...
   Документация Яндекса предлагает отмечать заодно и `webmaster:verify`, но он
   нужен только для подтверждения прав на сайт — а эта операция из форка
   вырезана. По принципу наименьших привилегий берём один `hostinfo`.
   Если после подключения `ywm_submit_recrawl` или `ywm_add_sitemap` вернут
   403, выпустите токен заново, отметив оба права.
...
7. Подтвердите доступ — токен появится в адресной строке и на странице.
   Он длинный, вида `y0_AgAAAA...`, и **действует 6 месяцев** — потом просто
   пройдите по той же ссылке ещё раз.
